...
Monte Carlo data (already-generated Monte Carlo data from a previous run should be included in this
...
# The additive model results are processed directly in additive_model.sh
```
...
The .sh files in step 2 runs the script in parallel **over `est_var`**
```bash
...
python check_monte_carlo.py \
--est_var kfr_top20_black_pooled_p25 \ # Outcome variable being P(top 20 | Black, pooled, parents at 25th percentile)
--seed_number 381 \ # The seed used is 94301 + (seed_number mod seed_range). In this case we check seed 94682
--simulator_name npmle_by_bins # Simulator name = npmle_by_bins, coupled_bootstrap-0.9, weibull
```
...
`data/simulated_posterior_means`. I have found that different hardware would only agree up
